...
import { Budget, Category, Type, CampaignName } from "../../ldm/full";
...
export const DrillWithExternalDataExample = () => {
    ...
    const getFilters = (state, location) => {
        ...
        if (state) {
            filters.push(
                newPositiveAttributeFilter(Category.attribute, {
                    uris: [state.uri],
                }),
            );
        }
        if (location) {
            filters.push(
                newPositiveAttributeFilter(Type.attribute, {
                    uris: [location.uri],
                }),
            );
        }
    ...
    const stateTable = (
        <div style={{ height: 200 }} className="s-state-table">
            <PivotTable
                measures={[Budget.Avg]}
                rows={[Category.attribute]}
                drillableItems={[HeaderPredicates.identifierMatch(attributeIdentifier(Category.attribute))]}
    ...
    const employeeTable = (
        <div style={{ height: 300 }} className="s-employee-table">
            <PivotTable
                measures={[Budget.Avg]}
                rows={[CampaignName.attribute]}
                drillableItems={[
                    HeaderPredicates.identifierMatch(attributeIdentifier(CampaignName.attribute)),
                ]}
    ...
    const totalSalesChart = (
        <div style={{ height: 300 }} className="s-sales-chart">
            <ColumnChart
                measures={[Budget.Avg]}
                viewBy={Type.attribute}
                filters={salesTableFilters}
                drillableItems={[HeaderPredicates.identifierMatch(attributeIdentifier(Type.attribute))]}
